...
import { updateView } from '../api/PostFuncions';
import { getTracklist } from '../api/TrackList';
...
class Playlist extends Component {
    ...
    componentDidMount(){
        ...
        if(this.props.playlist.id !== undefined){
            this.getTracklist(this.props.playlist.id);
        }
    }
    ...
    render(){
        ...
        if(tracks.length > 0){
            ...
            .map((track, i)=> {         
                return (
                    <Tracks key={i} index={i} handleClickPlay={this.handleClickPlay} getTracklist={this.getTracklist}
